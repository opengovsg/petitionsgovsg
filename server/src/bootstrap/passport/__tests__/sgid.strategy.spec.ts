...
describe('sgidStrategy', () => {
  ...
  beforeAll(async () => {
    ...
    verifySgidCallback = _sgidStrategy.sgidCallbackWithName(User, privateKeyPem)
  })
  ...
  it('throws error if wrong private key', async () => {
    ...
    const verifySgidCallbackWithWrongPrivateKey =
      _sgidStrategy.sgidCallbackWithName(User, invalidPrivateKeyPem)
